...
struct FrameResult {
    ...
    const std::vector<double>& getVector(const std::string& key) const {
        ...
        auto it = values.find(key);
        if (it != values.end() && std::holds_alternative<std::vector<double>>(it->second))
...
struct AnalysisParams {
    ...
    size_t hopSize   = 0; // hopSize == 0 to ramki sie nie nakladaja, hopSize == frameSize to ramki sie nie nakladaja, hopSize < frameSize to ramki sie nakladaja
};
